...
def main():
    ...
    if Config.evaluate:
        ...
        checkpoint = torch.load(
            Config.evaluate, map_location=torch.device("cpu"))
    ...
    logger.info(
        f"finish training, best top1 acc: {best_acc:.2f}%, best top5 acc: {best_acc5:.2f}%, total training time: {training_time:.2f} hours")
        ...
        for i, loss_item in enumerate(Config.loss_list):
            ...
            if loss_type == "ce_family":
                tmp_loss = loss_alphas[i] * \
                    criterion[i](stu_outputs[-1], labels)
